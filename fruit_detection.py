...
def app():
    ...
    if net_key not in st.session_state:
        st.session_state[net_key] = cv2.dnn.readNet(model_path)
    net = st.session_state[net_key]
    ...
    img_file_buffer = st.file_uploader("📂 Chọn ảnh", type=["bmp", "png", "jpg", "jpeg", "tif", "gif"], key="fruit_upload")
    cols = st.columns(2)
    ...
    if img_file_buffer is not None:
        ...
        with cols[0]:
            ...
            st.image(frame, channels="BGR", width=640)

        with cols[1]:
            text_container = st.empty()
            img_container = st.empty()

        if st.button("🔍 Dự đoán", key="fruit_predict"):
